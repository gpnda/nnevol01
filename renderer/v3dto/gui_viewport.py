...
Адаптирован для Renderer v3dto (DTO архитектура).
"""
...
if TYPE_CHECKING:
    from .dto import RenderStateDTO, WorldStateDTO
...
class Viewport:
    ...
    # Расширено на всю ширину для v3dto архитектуры
    VIEWPORT_X = 5
    VIEWPORT_Y = 5
    VIEWPORT_WIDTH = 1240
    VIEWPORT_HEIGHT = 500
    ...
    def __init__(self):
        ...
        Не требует world, так как все данные передаются через RenderStateDTO.
        """
    ...
    def get_visible_range(self, world_width: int, world_height: int) -> tuple:
        """
        Возвращает диапазон видимых клеток карты.
        
        Args:
            world_width: ширина мира
            world_height: высота мира
        ...
        min_x = max(0, min_x)
        min_y = max(0, min_y)
        max_x = min(world_width, max_x)
        max_y = min(world_height, max_y)
        
        return min_x, max_x, min_y, max_y
    
    def get_creature_at_position(self, screen_pos: tuple, world_dto: 'WorldStateDTO') -> int | None:
        """
        Получить существо в точке клика.
        
        Args:
            screen_pos: (x, y) координаты на экране
            world_dto: WorldStateDTO с информацией о существах
    ...
    def _draw_raycast_dots(self, debug_dto) -> None:
        """Отрисовка точек raycasting для отладки."""
        if debug_dto is None or debug_dto.raycast_dots is None:
            return
        
        for dot in debug_dto.raycast_dots:
    ...
    def _draw_selected_creature_box(self, creature_dto) -> None:
        """
        ...
        Args:
            creature_dto: CreatureDTO с атрибутами x, y (float)
        """
        ...
        int_x, int_y = int(creature_dto.x), int(creature_dto.y)
        ...
        creature_pos = pygame.Vector2(creature_dto.x, creature_dto.y)
    ...
    def draw(self, screen: pygame.Surface, render_state: 'RenderStateDTO', font: pygame.font.Font = None) -> None:
        ...
        Args:
            ...
            render_state: RenderStateDTO со всеми необходимыми данными
        ...
        self._draw_cells(render_state.world)
        
        # Отрисовка raycast точек для отладки
        self._draw_raycast_dots(render_state.debug)
        
        # Отрисовка рамки вокруг выбранного существа
        if render_state.selected_creature is not None and render_state.selected_creature.is_valid:
            self._draw_selected_creature_box(render_state.selected_creature.creature)
