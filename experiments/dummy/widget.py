...
- По сути является виджетом таким-же как другие виджеты, и должен быть устроен также 
  как все виджеты в Renderer'е, с обработкой событий и отрисовкой на экране, 
  но при этом не должен иметь доступа к данным мира и логике эксперимента,
  Является самым чистым классом, потому что непосредственно получает данные в виде аттрибута метода
  draw() в виде DTO, и не имеет доступа ни к каким другим данным, кроме этих DTO.
- поверхность для отрисовки он тоже получает в виде аргумента метода draw(), и не имеет доступа 
  к другим поверхностям, кроме этой.
"""
...
class DummyExperimentWidget:
    ...
    def draw(self, screen: pygame.Surface, experiment_dto) -> None:
        ...
        current_tick = experiment_dto.tick_counter if hasattr(experiment_dto, 'tick_counter') else 'unknown'
        ...
        lines = [
            ...
            f"",
            f"Status:",
            f"  RUNNING {current_tick} ticks" if experiment_dto.is_running else "NOT RUNNING",
        ]
